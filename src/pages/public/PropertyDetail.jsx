...
const PropertyDetail = () => {
  const { id } = useParams();
  const { getPropertyById, loading } = useData();
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/properties" className="text-primary hover:text-primary-light font-medium">Back to Properties</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Link to="/properties" className="inline-flex items-center text-gray-500 hover:text-primary transition-colors mb-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 p-6 md:p-8">
            <div>
              <div className="relative rounded-xl overflow-hidden">
                <img
                  src={property.photos?.[0] || 'https://images.unsplash.com/photo-1564013799919-ab600027ffc6?w=800&h=600&fit=crop'}
                  alt={property.name}
              ...
              {property.photos?.length > 1 && (
                <div className="grid grid-cols-4 gap-2 mt-4">
                  {property.photos.slice(1, 5).map((photo, index) => (
                    <img key={index} src={photo} alt={`${property.name} ${index + 2}`} className="w-full h-20 object-cover rounded-lg" />
                  ))}
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <p className="text-3xl font-bold text-primary mt-4">${property.rate}</p>
