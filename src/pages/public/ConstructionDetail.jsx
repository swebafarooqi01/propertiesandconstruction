...
const ConstructionDetail = () => {
  const { id } = useParams();
  const { getConstructionById, loading } = useData();
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500 mb-6">The construction project you're looking for doesn't exist.</p>
          <Link to="/construction" className="text-primary hover:text-primary-light font-medium">Back to Construction</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Link to="/construction" className="inline-flex items-center text-gray-500 hover:text-primary transition-colors mb-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 p-6 md:p-8">
            <div>
              <div className="relative rounded-xl overflow-hidden">
                ...
                />
                {construction.hot && (
                  <span className="absolute top-4 left-4 px-4 py-2 bg-accent text-white text-sm font-medium rounded-full">Hot Project</span>
                )}
                {construction.sold && (
                  <span className="absolute top-4 right-4 px-4 py-2 bg-red-500 text-white text-sm font-medium rounded-full">Sold</span>
                )}
              </div>
              {construction.photos?.length > 1 && (
                <div className="grid grid-cols-4 gap-2 mt-4">
                  {construction.photos.slice(1, 5).map((photo, index) => (
                    <img key={index} src={photo} alt={`${construction.name} ${index + 2}`} className="w-full h-20 object-cover rounded-lg" />
                  ))}
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <p className="text-3xl font-bold text-primary mt-4">${construction.rate}</p>

              <div className="flex gap-3 mt-4">
                {construction.hot && <span className="px-3 py-1 bg-accent/10 text-accent-dark text-sm font-medium rounded-full">Featured</span>}
                {construction.sold ? (
              ...
              <div className="mt-8">
                ...
                <p className="text-gray-600 leading-relaxed">{construction.description}</p>
              </div>
